  ...
  try{
    ...
    if($stmt->rowCount()==1){     
      ...
      $message .= '<p>
			...
			$message .= '<a class="btn btn-primary btn-sm" href="'.$_SERVER['HTTP_HOST'].'/cocd/PHP/verify.php?id='.$id.'&code='.$code.'&email='.$email.'&time='.$time.'&role='.$role.'">click here to accept</a> ';
      $message .= '<p>
                      <br><br><br><br>
